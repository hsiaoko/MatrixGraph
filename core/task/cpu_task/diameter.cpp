#include "core/task/cpu_task/diameter.h"

#include <atomic>
#include <chrono>
#include <cstdint>
#include <ctime>
#include <execution>
#include <iostream>
#include <limits>
#include <numeric>
#include <vector>

namespace sics {
namespace matrixgraph {
namespace core {
namespace task {

using VertexID = sics::matrixgraph::core::common::VertexID;
using ImmutableCSRGraph =
    sics::matrixgraph::core::data_structures::ImmutableCSR;

namespace {

uint32_t BfsMaxDistanceUndirected(const ImmutableCSRGraph& g, VertexID src) {
  const VertexID n = g.get_num_vertices();
  constexpr uint32_t kInf = std::numeric_limits<uint32_t>::max();
  std::vector<uint32_t> dist(n, kInf);
  std::vector<VertexID> q;
  q.reserve(n);
  dist[src] = 0;
  q.push_back(src);
  std::size_t head = 0;
  uint32_t maxd = 0;
  while (head < q.size()) {
    const VertexID u = q[head++];
    const uint32_t du = dist[u];
    if (du > maxd) maxd = du;

    auto relax = [&](VertexID v) {
      if (dist[v] == kInf) {
        dist[v] = du + 1u;
        q.push_back(v);
      }
    };

    for (VertexID k = 0; k < g.GetOutDegreeByLocalID(u); ++k) {
      relax(g.GetOutgoingEdgesByLocalID(u)[k]);
    }
    for (VertexID k = 0; k < g.GetInDegreeByLocalID(u); ++k) {
      relax(g.GetIncomingEdgesByLocalID(u)[k]);
    }
  }
  return maxd;
}

}  // namespace

void Diameter::LoadData() {
  std::cout << "[Diameter] LoadData()" << std::endl;
  g_.Read(data_graph_path_);
}

void Diameter::ComputeUndirectedDiameter(const ImmutableCSRGraph& g) {
  const VertexID n = g.get_num_vertices();
  if (n == 0) {
    std::cout << "[Diameter] undirected diameter: 0 (empty graph)" << std::endl;
    return;
  }

  std::vector<VertexID> sources(n);
  std::iota(sources.begin(), sources.end(), static_cast<VertexID>(0));

  std::atomic<uint32_t> diameter{0};
  std::for_each(std::execution::par, sources.begin(), sources.end(),
                [&](VertexID s) {
                  const uint32_t ecc = BfsMaxDistanceUndirected(g, s);
                  uint32_t cur = diameter.load(std::memory_order_relaxed);
                  while (ecc > cur &&
                         !diameter.compare_exchange_weak(
                             cur, ecc, std::memory_order_relaxed,
                             std::memory_order_relaxed)) {
                  }
                });

  std::cout << "[Diameter] undirected diameter (exact, parallel BFS per "
               "vertex): "
            << diameter.load() << std::endl;
}

void Diameter::Run() {
  auto t0 = std::chrono::system_clock::now();
  LoadData();
  auto t1 = std::chrono::system_clock::now();
  ComputeUndirectedDiameter(g_);
  auto t2 = std::chrono::system_clock::now();

  std::cout << "[Diameter] LoadData() elapsed: "
            << std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0)
                       .count() /
                   static_cast<double>(CLOCKS_PER_SEC)
            << std::endl;
  std::cout << "[Diameter] Compute() elapsed: "
            << std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1)
                       .count() /
                   static_cast<double>(CLOCKS_PER_SEC)
            << std::endl;
}

}  // namespace task
}  // namespace core
}  // namespace matrixgraph
}  // namespace sics
