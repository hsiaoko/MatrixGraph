...
public:
  GPUBitmap(size_t size) { Init(size); }

  GPUBitmap(size_t size, uint64_t *data) { Init(size, data); }
  ...
  void Init(size_t size) {
    ...
    size_ = size;
    cudaMallocManaged(&data_, sizeof(unsigned long long) *
                                  (GPU_WORD_OFFSET(size_) + 1));
    cudaMemset(data_, 0,
  ...
  size_t GPUPreElementCount(size_t idx) const {
    ...
    matrix_count_kernel<<<dimGrid, dimBlock, 0, stream>>>(idx, data_,
                                                          count_ptr);
    cudaStreamSynchronize(stream);
    cudaStreamDestroy(stream);
